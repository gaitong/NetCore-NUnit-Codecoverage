...
namespace NUnitTest
{
    public class LicenseRenewServiceTest
    {
        ...
        {
            //mock repository การจำลอง db
            autoMock.Mock<ILicenseRenewRepository>().Setup(m => m.GetAll()).Returns(GetLicenseRenews());
            //ยัดเหยียดสิ่งแปลกปลอม
            var licesnseRenewService = autoMock.Create<LicenseRenewService>();

            var licenseRenews = licesnseRenewService.GetAll().ToList();
